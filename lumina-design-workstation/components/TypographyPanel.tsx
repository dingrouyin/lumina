...
const TypographyPanel: React.FC<TypographyPanelProps> = ({
    ...
    const panelContent = (
        <div ref={panelRef} className={`typography-panel-inner ${isDrawer ? 'w-[280px] max-h-[70vh] border-0 shadow-none' : 'w-[280px] m-4 rounded-2xl border border-violet-100 shadow-[0_16px_40px_rgba(0,0,0,0.1)] overflow-hidden'} bg-white/40 flex flex-col pointer-events-auto transition-shadow duration-300 ${!isDrawer ? 'hover:shadow-[0_20px_50px_rgba(0,0,0,0.15)] bg-white/90 backdrop-blur-md' : ''} ${isDraggingState && !isDrawer ? 'opacity-80 scale-[1.02] cursor-grabbing shadow-2xl ring-2 ring-violet-500/50' : ''}`}
        ...
        >
            <div
                className={`flex items-center justify-between p-4 border-b border-gray-50 cursor-grab active:cursor-grabbing select-none`}
            ...
            >
                <h3 className="text-xs font-black text-gray-800 uppercase tracking-widest flex items-center gap-1.5 pointer-events-none select-none">
                    <i className="fa-solid fa-font text-violet-500"></i>排版控制
                </h3>
                <div className="flex items-center gap-2" onPointerDown={(e) => e.stopPropagation()}>
                    ...
                    {onClose && (
                        <button onClick={onClose} className="text-gray-400 hover:text-red-500 transition-colors pointer-events-auto p-1.5">
            ...
            <div className={`space-y-5 p-4 ${isDrawer ? 'overflow-y-auto' : ''}`}>
                ...
                <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-gray-500 flex items-center gap-1.5"><i className="fa-solid fa-palette w-3 text-center"></i>颜色</span>
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            onPointerDown={(e) => e.stopPropagation()}
                            className="w-16 text-xs text-center font-mono border border-gray-200 rounded py-1 text-gray-600 focus:outline-none focus:border-violet-400 uppercase bg-white/50 hover:bg-white transition-colors"
                        />
                        <input
                            type="color"
                            value={firstSelectedText?.color || '#333333'}
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-gray-500 flex items-center gap-1.5"><i className="fa-solid fa-text-height w-3 text-center"></i>字号</span>
                        <span className="text-xs font-mono text-violet-700 bg-violet-50 px-1.5 py-0.5 rounded shadow-sm">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-gray-500 flex items-center gap-1.5"><i className="fa-solid fa-arrows-up-down w-3 text-center"></i>行高</span>
                        <span className="text-xs font-mono text-violet-700 bg-violet-50 px-1.5 py-0.5 rounded shadow-sm">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-gray-500 flex items-center gap-1.5"><i className="fa-solid fa-text-width w-3 text-center"></i>字距</span>
                        <span className="text-xs font-mono text-violet-700 bg-violet-50 px-1.5 py-0.5 rounded shadow-sm">
    ...
    return isDrawer ? panelContent : (
        <div className={`typography-panel-root absolute z-40 ${isDocked && !isDraggingState ? 'right-[72px]' : ''}`}>
            ...
            {!isDrawer && isDraggingState && isDocked && (
                <div className="fixed top-1/2 right-[72px] -translate-y-1/2 w-[280px] h-[300px] border-2 border-violet-500/50 bg-violet-100/20 rounded-2xl shadow-[0_0_30px_rgba(139,92,246,0.2)] pointer-events-none z-[-1] transition-all duration-300 backdrop-blur-sm">
